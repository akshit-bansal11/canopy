    ...
    >
            ...
            <div className="flex items-center justify-between mt-1 h-6">
              <div className="flex flex-wrap gap-1.5">
                {(tags || []).map((tag) => (
                  <span
                    key={tag}
                    className="text-[10px] font-mono text-[var(--text-muted)] bg-[var(--bg-main)] px-1.5 py-0.5 rounded border border-[var(--border-main)]"
                  >
                    #{tag}
                  </span>
                ))}
      ...
      {!isEditing && (
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-[var(--bg-card)]/80 backdrop-blur-sm rounded p-1">
          ...
          <button
            onClick={() => onDelete(id)}
            className="p-1.5 text-[var(--text-muted)] hover:text-red-400 hover:bg-red-950/50 rounded"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
